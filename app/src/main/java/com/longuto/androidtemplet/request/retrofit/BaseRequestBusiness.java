package com.longuto.androidtemplet.request.retrofit;

import com.longuto.androidtemplet.request.NetworkConstant;
import com.longuto.androidtemplet.request.data.BaseResponse;
import com.longuto.androidtemplet.request.exception.ApiException;
import com.longuto.androidtemplet.request.retrofit.subscriber.ProgressSubscriber;
...
public abstract class BaseRequestBusiness {
     ...
     * @return
     */
      private static <T> ObservableTransformer<BaseResponse<T>, T> handleResult() {
          ...
          public Observable<T> apply(Observable<BaseResponse<T>> tObservable) {
            return tObservable.flatMap(new Function<BaseResponse<T>, ObservableSource<T>>() {
              ...
              public Observable<T> apply(BaseResponse<T> result) {
                //成功后交给界面处理
                if (result.getStatus() == NetworkConstant.SUCCESS_CODE) {
                    return createData(result.getData());
                } else {
                    //统一处理服务器返回值非正常结果
                    LogUtils.mvp_frame_e("统一处理服务器返回值非正常结果apply: " + result.getMessage());
     ...
     * @param ob
     */
    public void toSubscribeString(Observable<String> ob) {
        ob.subscribeOn(Schedulers.io())
                ...
                .subscribe(new Observer<String>() {
                    ...
                    public void onNext(String s) {
                        LogUtils.mvp_frame_e("当前返回的json字符串是：" + s);
                    }

                    @Override
                    public void onError(Throwable e) {
                        LogUtils.mvp_frame_e("当前错误原因：" + e.getMessage());
                    }
